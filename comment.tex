...
throughput of RT-PCR tests \cite{DorfmanYuvalDor, PoolSize30,
  BayesianDorfman, MatrixPooling, LionDorfman}. These techniques are
called \emph{pooling} and originated in the seminal work of Dorfman
pooling \cite{DorfmanOriginal, DorfmanYuvalDor}: Select $N$
individuals and perform a single RT-PCR test on their combined
(\emph{pooled}) samples. If the pooled test yields a positive result
--- test each individual separately. The throughput efficiency of
