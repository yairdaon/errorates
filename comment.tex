...
\begin{abstract}
pooling is a popular strategy for increasing SARS-CoV-2 testing
throughput. One popular pooling scheme is Dorfman pooling: test $N$
individuals simultaneously. If the test is positive, each individual
is then tested separately. Most analyses of pooling scheme error rates
assume that including extra infected samples in a pooled test has no
effect on the pooled test outcome. We refute this assumption with
...
for an infected individual) of Dorfman pooling via a our model. We
show that the false-negative rates under Dorfman pooling increase when
the prevalence of infection decreases. Thus, pooling is riskiest
exactly when it is most efficient. Conclusion/ summary.
