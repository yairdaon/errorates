...
pooling schemes is explicitly taking the intrinsic PCR error rates
into account. As we have demonstrated above, analyzing the expected
error rates of pooling schemes under a realistic probabilistic error
model is readily achievable. Designing pooling schemes which optimize
testing while accounting for error rates is, however, far from
trivial. Nonetheless, various algorithms explicitly incorporate error
rates into their pooling optimization processes (REFS), including a
...
performs pooling schemes, updating its information based on previous
results. In doing so, our pooling scheme shows potential in terms of
both reducing false-negative rates, as well as increasing efficiency,
