...
Our analysis has several implications: Previously estimated
optimal Dorfman pool size \cite{OptimalDorfmanPool} should be
revisited in general and specifically for the case of SARS-CoV-2, as it is influenced by the
prevalence of infection. The error rates of other pooling schemes in
use should also be revisited \cite{BayesianDorfman, Kim}. Moreover, studies that seek precise error
models should be conducted for other pathogens as well, since the
standard error model could prove flawed. Specifically, all assumptions should be scrutinized,
...
in \cite{KitComparison}. These assumptions lead to the following model,
...
different prevalence values. We consider a specificity of
...
sensitivity and prevalence values \cite{KitComparison,
  ...
  FalsePositiveEstimate}. We observe that the increase in the scheme false-negative rate
(compared to $1-\Se$), expressed by $\err{exact}$, is at least
$60\%$ (Figure \ref{fig1}).
...
\section*{Discussion}
In this study a novel probabilistic model is developed to capture realistic features of the outcomes of pooled RT-PCR tests, parameterized for
...
nursing homes \cite{DorfmanYuvalDor},
...
error rates. As illustrated above, analyzing the expected
